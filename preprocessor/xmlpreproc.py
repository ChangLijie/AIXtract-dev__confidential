...
class XmlPreProcessor(BasePreprocessor):
    ...
    def split_texts_by_center_segment(
        ...
        Returns:
            Tuple[List[str], List[str]]: Two lists of XML strings, A and B.
        """
        sorted_lines = self.sort_text_elements_by_top(xml_text_lines)
        ...
        A = [x["line"] for x in parsed if x["top"] <= split_top]
        B = [x["line"] for x in parsed if x["top"] > split_top]
    ...
    def process(self) -> Tuple[List[str], List[str]]:
        ...
        sorted_elements = self.sort_text_elements_by_top(self.data)

        # Remove duplicates
        unique_elements = self.deduplicate_text_elements_from_strings(sorted_elements)

        # Split into two segments
        A, B = self.split_texts_by_center_segment(unique_elements)
...
if __name__ == "__main__":
    ...
    xml_preprocessor = XmlPreProcessor(data)
    processed_data_a, processed_data_b = xml_preprocessor.process()
    for line in processed_data_a:
        print(line)
    print(f"\n{'-' * 50}\n")
    for line in processed_data_b:
        print(line)
