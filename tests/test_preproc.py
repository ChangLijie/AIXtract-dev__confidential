...
class TestXmlPreProcessor(unittest.TestCase):
    def setUp(self):
        ...
        self.xml_preprocessor = XmlPreProcessor(data=self.xml_data)

    def test_process(self):
        preprocessed_data_a, preprocessed_data_b = self.xml_preprocessor.process()

        # Check if the second output is a list
        self.assertIsInstance(preprocessed_data_a, list)
        # Test if the list is not empty
        self.assertGreater(
            len(preprocessed_data_a), 0, "Parsed data should not be empty"
        )
        # Test if the elements are strings
        self.assertTrue(
            all(isinstance(fragment, str) for fragment in preprocessed_data_a)
        )
        # Check if the second output is a list
        self.assertIsInstance(preprocessed_data_b, list)
        # Test if the list is not empty
        self.assertGreater(
            len(preprocessed_data_b), 0, "Parsed data should not be empty"
        )
        # Test if the elements are strings
        self.assertTrue(
            all(isinstance(fragment, str) for fragment in preprocessed_data_b)
        )
